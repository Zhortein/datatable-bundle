...
final class DoctrineGlobalSearchFunctionalTest extends FunctionalTestCase
{
    private ?EntityManagerInterface $entityManager = null;

    public function test_it_searches_string_columns(): void
    {
        $this->bootDoctrineAndLoadFixtures();

        $result = $this->createProvider()->getData(
            $this->createDefinition(),
            DatatableRequest::create(searchQuery: 'bob', pageSize: 10),
        );
    ...
    public function test_it_searches_case_insensitively(): void
    {
        ...
        $result = $this->createProvider()->getData(
            $this->createDefinition(),
            DatatableRequest::create(searchQuery: 'ALICE', pageSize: 10),
        );
    ...
    public function test_it_searches_numeric_columns_when_query_is_numeric(): void
    {
        $this->bootDoctrineAndLoadFixtures();

        $result = $this->createProvider()->getData(
            $this->createDefinition(),
            DatatableRequest::create(searchQuery: '2', pageSize: 10),
        );
    ...
    public function test_it_ignores_non_searchable_columns(): void
    {
        $this->bootDoctrineAndLoadFixtures();

        $definition = $this->createDefinitionWithNonSearchableEmail();
    ...
    public function test_it_combines_permanent_filters_and_search(): void
    {
        $this->bootDoctrineAndLoadFixtures();

        $definition = $this->createDefinition();
        $definition->addPermanentFilter('e.enabled', \Zhortein\DatatableBundle\Enum\FilterOperator::Equals, true);

        $result = $this->createProvider()->getData(
            $definition,
            DatatableRequest::create(searchQuery: 'charlie', pageSize: 10),
        );
    ...
    private function createDefinitionWithNonSearchableEmail(): DatatableDefinition
    {
        $definition = new DatatableDefinition('doctrine-users');

        $definition
            ->setEntityClass(DoctrineUser::class)
            ->addColumn('e.id', searchable: false)
            ->addColumn('e.email', label: 'Email', searchable: false)
            ->addColumn('e.displayName', label: 'Display name', searchable: false)
        ;
